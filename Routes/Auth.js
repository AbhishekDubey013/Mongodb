...
router.get('/adh', async (req, res) => {
  try {
    ...
    const filteredQas = await AT.find({ PK: PK }, { mobileNumber: 1, dataArray: 1 }); // This will fetch all documents from the "qas" collection where flag is 'Y' and only return the 'mobileNumber'
...
router.get('/pdh', async (req, res) => {
  ...
  async function fetchAndStoreBitcoinPrice() {
    try {
        ...
        const existingEntry = await BitcoinPrice.findOne({
            'time.updatedISO': { $gte: new Date().setHours(0,0,0,0), $lt: new Date().setHours(23,59,59,999) }
        });

        if (existingEntry) {
            console.log('Updating existing entry');
            await BitcoinPrice.updateOne({ _id: existingEntry._id }, { $set: data });
        } else {
            console.log('Creating new entry');
            const newBitcoinPrice = new BitcoinPrice(data);
            await newBitcoinPrice.save();
        }
    } catch (error) {
        console.error('Error:', error);
    }
...
cron.schedule('0 * * * *', function() {
  console.log('Running scheduled task to fetch Bitcoin price');
  fetchAndStoreBitcoinPrice();
});

fetchAndStoreBitcoinPrice(); // Fetch Bitcoin price immediately on startup
